...
import {
  ...
  CheckCircle, XCircle, Eye, Download, X, Copy, StarOff,
  BookmarkPlus, FolderOpen, Clock
} from 'lucide-react';
import { Connection, FavoriteSection } from '@/types';
import ConnectionModal from './ConnectionModal';
import { saveAs } from 'file-saver';
import Editor from '@monaco-editor/react';

// ─── Modal: guardar favorito (nombre + sección) ──────────────────────────────
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className={`w-full max-w-sm rounded-2xl shadow-2xl border p-6 flex flex-col gap-4 ${
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          {/* Sección */}
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium opacity-70">Sección <span className="text-red-400">*</span></label>
...
export default function Sidebar() {
  ...
  const openFavorite = (favId: string, name: string, sql: string) => {
    ...
    if (existing) {
      ...
      addTab({ id: crypto.randomUUID(), title: name, query: sql });
    }
    runFavorite(favId);
  };
