...
import { TABLAS_SQL_CONTENT } from '@/constants/tablasSql';
...
export default function SqlInstructionsModal({ isOpen, isDark, onClose }: SqlInstructionsModalProps) {
  const [copied, setCopied] = useState(false);

  if (!isOpen) return null;

  const handleCopy = () => {
    navigator.clipboard.writeText(TABLAS_SQL_CONTENT).then(() => {
      setCopied(true);
      ...
      >
        ...
        <div className={`flex items-center gap-3 px-5 py-4 border-b flex-shrink-0
          ...
          <div className="flex-1 min-w-0">
            <h2 className="font-bold text-sm">Instrucciones de Creación de Tablas</h2>
            <p className="text-xs opacity-50 mt-0.5">
              Ejecuta este script en tu base de datos Oracle para habilitar la sincronización de favoritos
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className={`px-5 py-3 flex items-start gap-3 border-b text-xs flex-shrink-0
          ...
          <div className={`leading-relaxed ${isDark ? 'text-amber-300/80' : 'text-amber-800'}`}>
            <span className="font-semibold">Las tablas necesarias no existen en esta base de datos.</span>
            {' '}Copia el script, conéctate a tu base de datos Oracle con SQL*Plus, SQL Developer u otra herramienta,
            y ejecútalo para crear las tablas <span className="font-mono font-semibold">TKR_FAVORITOS</span> y{' '}
            <span className="font-mono font-semibold">TKR_FAVORITOS_SECCIONES</span>.
          </div>
        </div>
