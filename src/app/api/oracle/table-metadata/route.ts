...
export async function POST(req: Request) {
  try {
    ...
    // 2. Fetch relationships between selected tables (including external ones)
    const relsSql = `
      SELECT 
    ...
    // 6. Fetch Check constraints and Unique constraints
    const constraintsSql = `
      ...
      WHERE 
          c.constraint_type IN ('U', 'C')
          AND c.table_name IN (${bindPlaceholders})
      ORDER BY 
          c.table_name, c.constraint_name, cc.position
    `;
    const constraintsResult = await executeOracleQuery(connection, constraintsSql, binds);
    ...
    const constraints = (constraintsResult.rows || []).map((row: any) => ({
      tableName: row.TABLE_NAME || row.table_name,
      constraintName: row.CONSTRAINT_NAME || row.constraint_name,
      constraintType: row.CONSTRAINT_TYPE || row.constraint_type,
      columnName: row.COLUMN_NAME || row.column_name,
      searchCondition: row.SEARCH_CONDITION || row.search_condition
    }));
    ...
    return NextResponse.json({ success: true, columns, relations, indexes, primaryKeys, triggers, constraints });
  } catch (error: any) {
